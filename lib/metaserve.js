...
(function() {
  var BASE_DIR, CONFIG_FILE, CSS_COMPILER, DEFAULT_BASE_DIR, DEFAULT_COMPILERS, HOST, HTML_COMPILER, IS_SPECIFIC_CONFIG, JS_COMPILER, PORT, VERBOSE, app, argv, compilers, config, de_res, defaults, e, express, file_path, fs, isArray, isString, metaserve_compile, metaserve_middleware, path, url, util;
  ...
  metaserve_compile = function(all_compilers, file_path, config, context, cb) {
    var base_dir, compiler, compiler_config, compiler_context, compilers, ext, filename, filename_stem, matched, path_match, _i, _len;
    if (VERBOSE) {
  ...
  if (require.main === module) {
    ...
    config.base_dir || (config.base_dir = BASE_DIR);
      ...
      metaserve_compile(compilers, file_path, config, {}, function(err, response) {
        ...
        if ((response != null ? response.compiled : void 0) != null) {
          if (typeof argv.out === 'boolean') {
            ...
            bounced_filename = argv.out || path.join(config.base_dir, file_path + '.bounced');
