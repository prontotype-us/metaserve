...
(function() {
  var BASE_DIR, CONFIG_FILE, CSS_COMPILER, DEFAULT_BASE_DIR, DEFAULT_COMPILERS, HOST, HTML_COMPILER, JS_COMPILER, PORT, VERBOSE, app, argv, bouncedExtension, compilers, config, de_res, e, express, filename, fs, isArray, isString, metaserve, metaserve_compile, options, url, util;
  ...
  metaserve_compile = function(file_url, options, cb) {
    var base_dir, compiler, compilers, ext, filename, filename_stem, matched, url_match, _i, _len, _ref, _ref1;
    ...
    if (filename = argv.bounce) {
      ...
      metaserve_compile(filename, options, function(err, response) {
        ...
        if ((response != null ? response.compiled : void 0) != null) {
          ...
          return console.log("[metaserve] Bouncing failed, make sure ." + filename + " exists");
        }
      });
    } else {
      app = express();
